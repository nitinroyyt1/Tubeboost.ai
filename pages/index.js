...
export default function Home() {
  ...
  async function handleAction(action) {
    ...
    const prompts = {
      tags: `YouTube Video URL: ${trimmed}\n\n18 high-ranking SEO tags generate karo. Sirf numbered list.`,
      transcript: `YouTube Video URL: ${trimmed}\n\n3 key takeaway bullet points aur ek SEO description do.`,
      title: `YouTube Video URL: ${trimmed}\n\n3 click-worthy SEO titles with reason. Numbered list.`,
      description: `YouTube Video URL: ${trimmed}\n\nFull SEO YouTube description with hook, keywords, hashtags, CTA.`,
    };
    const titles = { tags: 'SEO TAGS', transcript: 'VIDEO SUMMARY', title: 'TITLE OPTIMIZER', description: 'DESCRIPTION' };
  ...
  function copyAll() {
    const text = resultType === 'tags' ? resultData.join(', ') : resultData;
    ...
    <>
        ...
        <div className="container">
          ...
          {resultType && (
            <div className="result-area">
              ...
              {resultType === 'tags' && Array.isArray(resultData) && (
                <div className="tags-container">
                  {resultData.map((tag, i) => (
                    <span key={i} className="tag-chip" style={{animationDelay:`${i*0.04}s`}} onClick={() => navigator.clipboard.writeText(tag)}>{tag}</span>
                  ))}
                </div>
              )}
              ...
              {resultType === 'dl' && <div className="dl-info"><div className="dl-note"><strong>Pro Feature</strong><br/><br/>Video/Audio download ke liye backend chahiye.</div></div>}
      ...
      <style jsx global>{`
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        input[type="text"]{width:100%;background:var(--dark3);border:1px solid var(--border);border-radius:10px;padding:14px 18px;color:var(--text);font-size:15px;outline:none}
        ...
        .action-btn{display:flex;align-items:center;gap:10px;background:var(--dark3);border:1px solid var(--border);border-radius:10px;padding:13px 16px;color:var(--text);cursor:pointer;transition:all .2s;text-align:left;width:100%}
        .action-btn:hover{border-color:var(--red);background:rgba(255,0,0,.06)}
        .btn-icon{font-size:20px;width:32px;text-align:center}
        ...
        .copy-btn{background:rgba(255,255,255,.05);border:1px solid var(--border);border-radius:6px;padding:6px 14px;color:var(--text);font-family:'JetBrains Mono',monospace;font-size:11px;cursor:pointer}
        ...
        .result-content{padding:20px;font-size:14px;line-height:1.9}
        ...
        .tag-chip:hover{background:rgba(255,0,0,.18);color:#fff}
        ...
        .thumb-actions{display:flex;gap:10px;justify-content:center;margin-top:14px}
        .thumb-btn{background:var(--dark3);border:1px solid var(--border);border-radius:8px;padding:8px 16px;color:var(--text);font-size:12px;font-family:'JetBrains Mono',monospace;text-decoration:none}
        ...
        @media(max-width:600px){.buttons-grid{grid-template-columns:1fr}}
      `}</style>
    </>
  );
                                            }
